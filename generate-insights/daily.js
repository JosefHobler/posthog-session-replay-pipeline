...
async function ensurePromptCache(aiClient, model, prompt, tracking = {}) {
  ...
  const ttlEnv = process.env.GEMINI_CACHE_TTL || "600s"; //
...
export async function getSessionAnalysesByEmail(email, limit = null) {
  try {
    ...
    let analysesQuery = `
      ...
      `;
    console.log("fewfwnfwjn", targetDay);
    const queryParams = [user.id, targetDay];
...
async function main() {
  ...
  for (const { email } of flaggedUsers) {
    ...
    // Initialize a cost tracking session for this user
   ...
   return */
    ...
    if (merged === undefined) {
      ...
      /*   const tasks = [
            {
              key: "moneyLoss",
              systemInstruction:
                `Show me where I'm losing money. Only consider insights that appear in 2 or more sessions.`,
              contents:
                `Show me where I'm losing money. Only consider insights that appear in 2 or more sessions.`
            },
            {
              key: "actionPlan",
              systemInstruction:
                `Create my action plan. Only consider insights that appear in 2 or more sessions.`,
              contents:
                `Create my action plan. Only consider insights that appear in 2 or more sessions.`
            },
            {
              key: "bugs",
              systemInstruction:
                `Was there any bug or error that occurred in 2 or more sessions? If not, return an empty array.`,
              contents:
                "Was there any bug or error that occurred in 2 or more sessions? If not, return an empty array."
            },
            {
              key: "priorities",
              systemInstruction:
                `What should be my top priorities? Only consider insights that appear in 2 or more sessions.`,
              contents:
                "What should be my top priorities? Only consider insights that appear in 2 or more sessions."
            },
          ]; */
      ...
      const tasks = [
        {
          key: "errors",
          systemInstruction: `Was there any error that occurred? If not, return an empty array.`,
          contents:
            "Was there any error that occurred? If not, return an empty array.",
        },
        {
          key: "bugs",
          systemInstruction: `Was there any bug that occurred? If not, return an empty array.`,
          contents:
        ...
        {
          key: "revenueLeaks",
          systemInstruction: `Show me where I'm losing money.`,
          contents: `Show me where I'm losing money.`,
        },
        {
          key: "topPriorities",
          systemInstruction: `What should be my top priorities?`,
          contents: "What should be my top priorities?",
        },
 ...
 */
      ...
      // Run tasks sequentially and add an ignore list of prior titles
      const parsedArrays = [];
        ...
        try {
          const res = await generateContentWithRetry(
            ai,
            {
              ...
              contents: `${t.systemInstruction}${ignoreBlock}`,
            },
            3,
            apiKeys,
            {
          ...
          if (newAsSchemaInput.length === 0 && candidatesObjects.length === 0) {
            ...
            const respMerge = await generateContentWithRetry(
              ai,
              {
                ...
                systemInstruction:
                  "Please create one complete action plan from these two lists.",
                contents: `Please create one complete action plan from these two lists.
...
${JSON.stringify(mergedWorking)}
    ...
    try {
      ...
      // This helps ensure evidenceSessions persist across USE_FILE/USE_FILE2/USE_FILE3 runs
      try {
      ...
      try {
        ...
        for (const insight of Array.isArray(result) ? result : []) {
          ...
          // Only attach if not already present (e.g., when loaded from enriched file)
          ...
          try {
            ...
            for (const insight of Array.isArray(result) ? result : []) {
              if (!insight || !Array.isArray(insight.evidenceSessions))
                continue;
              for (const sess of insight.evidenceSessions) {
                if (Array.isArray(sess.insightClips)) {
                  clipsBySessionId[sess.idVar] = sess.insightClips;
                }
              }
            }
            fs2.writeFileSync(
      ...
      const resultArgument = result.map((value) => {
        ...
        if (Array.isArray(value.evidenceSessions)) {
          for (const sess of value.evidenceSessions) {
            ...
            // If we skipped sessionTasks due to old analysisId, keep the PostHog URL without times
            /*  if (clips.length === 0) {
              const analysisIdNum = Number(sess?.analysisId);
              if (Number.isFinite(analysisIdNum) && analysisIdNum <= 15176) {
                evidenceObjects.push({
